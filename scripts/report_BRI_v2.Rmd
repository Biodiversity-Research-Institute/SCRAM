...
```{r, table8, echo=FALSE, warning=FALSE, message = FALSE}

num_species <- length(params$model_output[['CRSpecies']])
monthlysppgrid_allspp <- data.frame()

for(e in 1:num_species){
  spp_code <- params$model_output[['CRSpecies']][e]
  Species <- gsub("_", " ", spp_code)
  monthlySpeciesGrid <- params$model_output$monthlySpeciesGridCount[[spp_code]]
  
  spp_count_out_melt <- reshape2::melt(monthlySpeciesGrid, value.name = "monthly_count")
  spp_count_out_melt <- spp_count_out_melt %>% 
    dplyr::rename("Month"="variable")
  spp_count_out_melt$iter <- rep(seq(1:length(monthlySpeciesGrid$Jan)), 12)
  #calculate mean and 95% CI  
  monthlySpeciesGrid_summary <- spp_count_out_melt %>% 
    dplyr::group_by(Month) %>% 
    dplyr::summarise(`Mean count` = as.character(round(mean(monthly_count, na.rm = T), 3)), 
                     `Lower pred. interval` = as.character(round(quantile(monthly_count, c(0.025), na.rm=TRUE), 3)),
                     `Upper pred. interval` = as.character(round(quantile(monthly_count, c(0.975), na.rm=TRUE), 3))) %>% 
    replace_na(list(`Mean count`="", `Lower pred. interval`="", `Upper pred. interval`="")) %>% 
    mutate_if(is.character,
              str_replace_all, pattern = "NaN", replacement = "") %>% 
    mutate(Month = as.character(Month))
  monthlySpeciesGrid_summary <- cbind(Species, monthlySpeciesGrid_summary)
  monthlysppgrid_allspp <- rbind(monthlysppgrid_allspp, monthlySpeciesGrid_summary)
}

kbl(monthlysppgrid_allspp, booktabs=T, caption = "Monthly species count data (mean and 95 perc. range) at the wind farm location.", row.names = F) %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2:5,width = "0.75in")
...
for(q in 1:num_species){
  ...
  annual_rate <- annual_summary %>% 
    dplyr::mutate(mean = round(mean(annual_summary$sum_monthly, na.rm=TRUE), 3),
                   lower = round(quantile(annual_summary$sum_monthly, c(0.025), na.rm=TRUE), 3),
                   upper = round(quantile(annual_summary$sum_monthly, c(0.975), na.rm=TRUE), 3))
  
  annual_collision_rate_txt <- paste0("Total annual collision rate and 95 perc. prediction interval: ",annual_rate$mean, 
...
<!-- ```{r, fig4, fig.height = 4, echo=FALSE, warning=FALSE, message = FALSE, fig.cap="The mean number of collisions per month for each turbine model combination. A sample of 100 runs of the of simulation are plotted showing the typical variation among iterations. The iterations among this set that exceed the user-specified threshold for the number of collisions, if set, are shown in red; iterations below this threshold are shown in blue. Since the threshold is a yearly value, the threshold here is that value divided by 12. The annual and monthly mean (95 perc. prediction interval) for all iterations are shown at top (annual) and for each month to the right of the plot. "} -->
...
<!--     annual_collision_rate_melt$iter <- rep(seq(1:length(annual_collision_rate_melt$Jan)), 12) -->
...
<!--                        lower_coll_rate = quantile(coll_rate, c(0.025), na.rm=TRUE), -->
<!--                        upper_coll_rate = quantile(coll_rate, c(0.975), na.rm=TRUE)) %>%  -->
<!--       dplyr::mutate(mean_95CI =  paste(round(sum_monthly, 3), " (", round(lower_coll_rate, 3),  -->
...
<!--     #calculate annual summary of model runs, mean of sum of posteriors and then quantiles of these sums for predict interval -->
<!--     annual_summary <- annual_collision_rate_melt %>%  -->
<!--       dplyr::group_by(iter) %>%  -->
<!--       dplyr::summarise(sum_monthly = sum(coll_rate, na.rm = T)) -->

<!--     annual_collision_rate_txt <- paste("Mean annual rate and 95 perc. prediction interval: ",round(mean(annual_summary$sum_monthly, na.rm=TRUE), 3),  -->
<!--                                    " (", round(quantile(annual_summary$sum_monthly, c(0.025), na.rm=TRUE), 3),  -->
<!--                                    ", ", round(quantile(annual_summary$sum_monthly, c(0.975), na.rm=TRUE), 3), ")", sep = "") -->
