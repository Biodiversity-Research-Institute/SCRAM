...
kbl(bird_params_table, booktabs=T, caption = "Species input parameters (mean and 95 perc. range).") %>%
...
popn_tbl_compact <- popn_tbl_compact %>% select(c("Species", month.abb))
...
kbl(turb_params_table, booktabs=T, caption = "Wind farm input parameters (mean and 95 perc. range).") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2,bold=T,color="blue",width = "0.75in") %>% 
...
colnames(turb_params_table2) <- c("Species", "Turbine Power (MW)","Prop. upwind", "Rotor speed (rpm)", "Pitch (radians)", "Farm width (km)", "Lat.", "Long.")
kbl(turb_params_table2, booktabs=T, caption = "Wind farm input parameters (mean and 95 perc. range).") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
...
colnames(turb_params_table_pt1) <- c("Species", "Turbine Power (MW)", "Jan Op.", "Feb Op.", "Mar Op.", "Apr Op.", "May Op.", "Jun Op.")
kbl(turb_params_table_pt1,booktabs=T, caption = "Monthly wind farm operational data (mean and 95 perc. range) is given for each wind farm specification.") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
...
colnames(turb_params_table_pt2) <- c("Species", "Turbine Power (MW)", "Jul Op.", "Aug Op.", "Sep Op.", "Oct Op.", "Nov Op.", "Dec Op.")
kbl(turb_params_table_pt2, booktabs=T, caption = "Monthly wind farm operational data (mean and 95 perc. range) is given for each wind farm specification.") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2,bold=T,color="blue",width = "0.65in") %>% 
...
shut_up(print(basemaps::basemap_ggplot(ext = map_extent, map_service = "esri", map_type = "world_ocean_base") +
  ...
  ggtitle(paste(sub("_", " ", paste0(params$model_output[['CRSpecies']][1])), "annual occurrence probability\n  and wind farm location.")) +
  theme_bw() +
...
for(q in 1:num_species){
  for(i in 1:num_turb_mods){
    ...
    model_summary <- cbind(Species = spp, `Turbine Power (MW)` = turb, model_summary)
    all_models_summary <- rbind(all_models_summary, model_summary)
    
  }
}

kbl(all_models_summary, booktabs=T, caption = "The predicted mean and 95 perc. prediction intervals of the number of collisions per month and the total summed monthly number of collisions and 95% prediction interval. Results are not shown for months that do not have movement data.") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
    ...
    if(!is.null(params$model_output$monthCollsnReps_opt1)){
      # if(sum(params$model_output[[as.numeric(params$option)]][[params$model_output[['CRSpecies']][q]]][[i]], na.rm=TRUE)>0){
        ...
        if(length(which(params$species_labels[,q] == params$model_output[['CRSpecies']][q]))>0){
          main_label <- paste(params$species_labels[params$species_labels[,1] == params$model_output[['CRSpecies']][q], 2], " (turbine model ", params$model_output[['Turbines']][i], " MW)", sep="")
        }else{
...
for(q in 1:num_species){
  ...
  if(length(which(params$species_labels[,1] == params$model_output[['CRSpecies']][q]))>0){
    main_label <- paste(
      ...
      " (turbine model ", params$model_output[['Turbines']][i], " MW)", sep=""
    )
  }else{
...
for(q in 1:num_species){
  for(i in 1:num_turb_mods){
    ...
    annual_lower_coll_rate <- quantile(annual_collision_rate_melt$coll_rate, c(0.025), na.rm=TRUE)
    annual_upper_coll_rate <- quantile(annual_collision_rate_melt$coll_rate, c(0.975), na.rm=TRUE)
    annual_collision_rate_txt <- paste("Mean annual rate and 95% prediction interval: ",round(annual_collision_rate, 2), 
                                       " (", round(annual_lower_coll_rate, 2), 
                                       ", ", round(annual_upper_coll_rate, 2), ")", sep = "")
...
if(num_turb_mods>1){
  ...
  for(i in 1:min(length(turbs_pair[1,]), 10)){
    ...
    #check to see if any difference, if there is one, plot
    if(data_diff > 0 | data_diff < 0){
