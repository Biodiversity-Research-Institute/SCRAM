...
## SCRAM run details 
<!-- load(file = "C:/Users/andrew.gilbert/Downloads/SCRAM_model_output_20230207_084610/SCRAM_model_output_20230207_084610.Rdata") -->
...
cat(paste0("SCRAM - the Stochastic Collision Risk Assessment for Movement version ", "\n",
          ...
          "Motus daily position modeled: ", params$model_type, "\n", 
...
if (params$model_type=="first"){
  spp_move_occur <- st_as_sf(get(paste0(spp, "_monthly_prob_BOEM_half_deg_trunc"))) %>% st_transform(3857)
} else {
  spp_move_occur <- st_as_sf(get(paste0(spp, "_monthly_prob_BOEM_half_deg_last"))) %>% st_transform(3857)
...
daily_cell_summary <- spp_daily_cell_count_melt %>%
  ...
  dplyr::mutate(month = as.character(month), `Est. daily num. of animals in the model cell` = paste0(mean, " (", lower, ", ", upper, ")")) %>%
  dplyr::mutate_if(is.character, str_replace, pattern = " \\(  ,   \\)", replacement = "")

nums_per_wf <- params$model_output$num_birds_WF_perday

spp_daily_wf_count_melt <- reshape2::melt(nums_per_wf, value.name = "daily_wf_count")
spp_daily_wf_count_melt <- spp_daily_wf_count_melt %>%
  dplyr::rename("month"="variable", "species"="L1", "turbine"="L2")
spp_daily_wf_count_melt$iter <- rep(seq(1:length(nums_per_wf[[1]][[1]][[1]])), 12)

#calculate mean and 95% CI
daily_wf_summary <- spp_daily_wf_count_melt %>%
  dplyr::group_by(species, turbine, month) %>%
  dplyr::summarise(mean = formatC(mean(daily_wf_count, na.rm = T),digits=3, format="g"),
                   lower = formatC(quantile(daily_wf_count, c(0.025), na.rm=TRUE),digits=3, format="g"),
                   upper = formatC(quantile(daily_wf_count, c(0.975), na.rm=TRUE), digits=3, format="g")) %>%
   dplyr::mutate_if(is.character, str_replace, pattern = "NaN", replacement = "") %>%
   dplyr::mutate_if(is.character, str_replace, pattern = "NA", replacement = "") %>%
   dplyr::mutate(month = as.character(month), `Est. daily num. of animals in the wind farm` = paste0(mean, " (", lower, ", ", upper, ")")) %>%
   dplyr::mutate_if(is.character, str_replace, pattern = " \\(  ,   \\)", replacement = "")
...
daily_crm_summary <- as.data.frame(cbind(cbind(daily_cell_summary[, c("species","turbine","month", "Est. daily num. of animals in the model cell")],
                                               daily_wf_summary[,"Est. daily num. of animals in the wind farm"]),
                                         daily_collisions_summary[, "Est. daily num. of collisions in the wind farm"]))
daily_crm_summary$species <- gsub("_", " ", daily_crm_summary$species)
colnames(daily_crm_summary) <- c("Species", "Turbine model", "month", "Est. daily num. of animals in the model cell",
                                 "Est. daily num. of animals in the wind farm", "Est. daily num. of collisions in the wind farm")

kbl(daily_crm_summary, booktabs=T, caption = "The estimated daily number of (95 perc. prediction intervals) animals in the model cell, animals in the wind farm, and collisions. Results are not shown for months that do not have movement data and does not mean that collisions could not occur in those months.") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>%
  ...
  column_spec(3,bold=T,color="blue",width = "0.5in") %>%
  column_spec(4:6,width = "0.75in")
...
monthly_summary <- model_out_melt %>% 
  ...
  mutate(month = as.character(month), `Est. num. of collisions` = paste0(mean, " (", lower, ", ", upper, ")")) %>% 
...
kbl(annual_summary, booktabs=T, caption = "The estimated monthly number (95 perc. prediction intervals) of collisions. Results are not shown for months that do not have movement data and does not mean that collisions could not occur in those months.") %>%
  kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>% 
  row_spec(0,bold=T, extra_css = "vertical-align:text-bottom	;") %>% 
  column_spec(1,bold=T,color="red", width = "1.25in") %>% 
  column_spec(2,bold=T,color="blue",width = "1in") %>% 
  ...
  for(i in 1:num_turb_mods){
    if(!is.null(params$model_output$monthCollsnReps)){
      ...
      if(length(which(params$species_labels[,q] == params$model_output[['CRSpecies']][q]))>0){
        main_label <- paste("Model option", params$option, "for species", 
                            params$species_labels[params$species_labels[,1] == params$model_output[['CRSpecies']][q], 2], " (turbine model ",
                            params$model_output[['Turbines']][i], ")", sep="")
      }else{
        main_label <- paste("Model option", params$option, "for species", params$model_output[['CRSpecies']][q],  " (turbine model ", 
                                                                          params$model_output[['Turbines']][i], ")", sep="")
      }
