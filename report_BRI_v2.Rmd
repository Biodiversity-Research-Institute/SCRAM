...
\begin{center}
\includegraphics[width=\textwidth]{SCRAM_logo_400px.png}
\end{center}
...
<!-- ![Biodiversity Research Institute](www/BRI_color_logo_no_words.png){width=1in} -->
<!-- ![University of Rhode Island](www/URI.png){width=1in}  -->
<!-- ![U.S. Fish and Wildlife Service](www/USFWS.png){width=1in}  -->
<!-- ![Bureau of Ocean Energy Management](www/BOEM.png){width=1in} -->
...
cat(paste0("SCRAM - the Stochastic Collision Risk Assessment for Movement version ", params$SCRAM_version,"\nwas run for ", params$iterations, " iterations using the: ", option_names[as.numeric(params$option)], "\n", 
          ...
          "The model run was started at: ",  format(params$run_start_time, "%a %b %d %X %Y"), "\n",
          "The model run was completed at: ",  format(params$run_end_time, "%a %b %d %X %Y"), "\n",
...
Table 1: For each parameter, the mean and 95% range (show in the parenthetical) is given for each species and turbine model combination.
...
kbl(bird_params_table,booktabs=T) %>%
  kable_styling(position="center", full_width=F, latex_options = "HOLD_position") %>% 
  row_spec(0,bold=T) %>% 
...
Table 2: For each parameter, the mean and 95% range (show in the parenthetical) is given for each species and turbine model combination.
...
kbl(turb_params_table,booktabs=T) %>%
  kable_styling(position="center", full_width=F, latex_options = "HOLD_position") %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2,bold=T,color="blue",width = "0.75in") %>% 
...
<br>
<br>
Table 3: For each parameter, the mean and 95% range (show in the parenthetical) is given for each species and turbine model combination.
...
kbl(turb_params_table2,booktabs=T) %>%
  kable_styling(position="center", full_width=F, latex_options = "HOLD_position") %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2,bold=T,color="blue",width = "0.75in") %>% 
...
<br>
<br>
Table 4: For each parameter, the mean and 95% range (show in the parenthetical) is given for each species and turbine model combination
...
kbl(turb_params_table_pt1,booktabs=T) %>%
  kable_styling(position="center", full_width=F, latex_options = "HOLD_position") %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
...
<br>
<br>
Table 5: For each parameter, the mean and 95% range (show in the parenthetical) is given for each species and turbine model combination.
...
kbl(turb_params_table_pt2,booktabs=T) %>%
  kable_styling(position="center", full_width=F, latex_options = "HOLD_position") %>% 
  row_spec(0,bold=T) %>% 
  column_spec(1,bold=T,color="red") %>% 
  column_spec(2,bold=T,color="blue",width = "0.65in") %>% 
...
Please see below for summary statistics for each of the stochastic parameters.


```{r, fig1, fig.height = 4, fig.width = 6.5, echo=FALSE, warning=FALSE, message = FALSE}
...
for(q in 1:num_species){
  for(i in 1:num_turb_mods){
    if(!is.null(params$model_output$monthCollsnReps_opt1)){
      # if(sum(params$model_output[[as.numeric(params$option)]][[params$model_output[['CRSpecies']][1]]][[1]], na.rm=TRUE)>0){
      #   NA_index <- which(is.na(params$model_output[[as.numeric(params$option)]][[params$model_output[['CRSpecies']][1]]][[1]][1,]))
      #   outvector <- round(rowSums(params$model_output[[as.numeric(params$option)]][[params$model_output[['CRSpecies']][1]]][[1]], na.rm = TRUE))
...
Figure 2: The predicted mean and 95% CI number of collisions per month. Results are not shown for months that do not have movement data. 

<br>

\newpage

```{r, fig3, fig.height = 4, fig.width = 6.5, echo=FALSE, warning=FALSE, message = FALSE}
...
Figure 3: The number of collisions per month for each species and turbine model combination. For each month, 100 iterations of the simulation results show typical variation among iterations. The iterations among this set that exceed the user-specified threshold for the number of collisions, if set, are shown in red; iterations below this threshold are shown in blue. The mean and 95% range among iterations are shown for each month to the right of the plot. 

<br>

\newpage

```{r, fig4, fig.height = 4, fig.width = 6.5, echo=FALSE, warning=FALSE, message = FALSE}
num_turb_mods <- length(params$model_output[['Turbines']])
if(num_turb_mods>1){
...
Figure 4: If multiple turbine models were run, this space shows histograms comparing the difference in the number of collisions per year between models. The y-axis shows the relative frequency (0-1) among iterations of each value for the difference between models, labeled by their power output. If the entire histogram lies above zero, the number of collisions was always higher for model listed first. 
